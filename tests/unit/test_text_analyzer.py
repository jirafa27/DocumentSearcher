...
class TestTextAnalyzer:
    ...
    def test_is_meaningful_word(self, analyzer):
        ...
        # Значимые слова
        assert analyzer.is_meaningful_word("дом") == True
        assert analyzer.is_meaningful_word("красивый") == True
        assert analyzer.is_meaningful_word("работать") == True
        
        # Служебные слова
        assert analyzer.is_meaningful_word("в") == False
        assert analyzer.is_meaningful_word("и") == False
        assert analyzer.is_meaningful_word("не") == False
        
        # Короткие слова
        assert analyzer.is_meaningful_word("я") == False
        assert analyzer.is_meaningful_word("") == False
    ...
    def test_all_query_words_present(self, analyzer):
        ...
        assert analyzer.all_query_words_present(fragment1, query) == True
        
        # Негативный случай
        fragment2 = "Красивая машина стоит во дворе"
        assert analyzer.all_query_words_present(fragment2, query) == False