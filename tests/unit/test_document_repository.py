...
from app.core.models.search import SearchResult, SearchDocument, SearchFragment, ContextInfo
...
class TestDocumentRepositoryUnit:
    ...
    async def test_search_exact_filters_matching_fragments(self, repository):
        ...
        mock_fulltext_results = [
            SearchResult(
                ...
                fragments=[
                    SearchFragment(text="точный запрос", context=mock_context),  # Точное совпадение
                    SearchFragment(text="точные запросы", context=mock_context), # Не точное
                    SearchFragment(text="Точный Запрос", context=mock_context),  # Еще одно точное
                ]
            ),
            SearchResult(
                document=mock_document,
                fragments=[
                    SearchFragment(text="контекст без совпадений", context=mock_context), 
    ...
    async def test_search_exact_no_matches(self, repository):
        ...
        mock_fulltext_results = [
            SearchResult(
                ...
                fragments=[
                    SearchFragment(text="точные запросы", context=mock_context),  # Не точное
                    SearchFragment(text="запрос точный", context=mock_context),   # Не точное
                ]
            )
        ]
    ...
    async def test_search_exact_with_user_filter(self, repository):
        ...
        await repository._search_exact(query, user_id, document_id, context_size_before, context_size_after)
