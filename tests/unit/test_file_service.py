...
from app.core.exceptions.file import FileTooLargeError, UnsupportedFileTypeError
...
class TestFileService:
    ...
    async def test_validate_file_file_too_large(self, file_service):
        ...
        with pytest.raises(FileTooLargeError):
            await file_service.validate_file(
                "test.docx", b"content" * 1024 * 1024 * 1024
            )
    ...
    async def test_extract_text_supported_types_mocked(self, file_service):
        ...
        with patch.object(
            file_service, "_extract_text_from_pdf", return_value="PDF text content"
        ) as mock_pdf, patch.object(
            file_service, "_extract_text_from_docx", return_value="DOCX text content"
        ) as mock_docx:
